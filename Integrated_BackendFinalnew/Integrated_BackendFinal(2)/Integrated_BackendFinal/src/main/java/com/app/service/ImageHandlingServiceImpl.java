...
public class ImageHandlingServiceImpl implements ImageHandlingService {
	@Autowired
	private StudentDetailsDao studDao;
	...
	public ApiResponse uploadImage(Long empId, MultipartFile image) throws IOException {
		// get emp from emp id
		Studentdetails stud = studDao.findById(empId).orElseThrow(() -> new ResourceNotFoundException("Invalid emp ID!!!!"));
		...
		return new ApiResponse("Image file uploaded successfully for student id " + empId);
	}

	@Override
	public byte[] downloadImage(Long empId) throws IOException {
		// get emp by id
		Studentdetails stud = studDao.findById(empId).orElseThrow(() -> new ResourceNotFoundException("Invalid student ID!!!!"));
		// emp found --> PERSISTENT
